...
#SBATCH --output=logs/CT/NP/sample/sample_2_CT_%A_%a_output.txt
#SBATCH --error=logs/CT/NP/sample/sample_2_CT_%A_%a_err.txt
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem-per-cpu=16G
#SBATCH --time=24:00:00
#SBATCH --array=0-40  # Array indices for 41 jobs
...
SAMPLE_INPUT_DIR="Projects/CT/samples/OFF/"
OUTPUT_DIR="Projects/CT/match/OFF/notpossible/"
PYTHON_SCRIPT="core/python/CT/NP/match_3_CT_NP.py"  # Updated to match the Python script name

# List all sample files in the input directory
...
if [ ${SLURM_ARRAY_TASK_ID} -lt ${total_files} ]; then
    # Get the sample file corresponding to the current task ID
