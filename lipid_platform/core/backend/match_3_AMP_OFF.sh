...
#SBATCH --output=core/backend/logs/match/OFF/match_3_AMP_%A_%a_output.txt
#SBATCH --error=core/backend/logs/match/OFF/match_3_AMP_%A_%a_err.txt
#SBATCH --ntasks=1
...
output_dir="Projects/AMP/match/OFF/"
...
if [ $SLURM_ARRAY_TASK_ID -lt ${#files[@]} ]; then
    ...
    python core/python/match_3_AMP_OFF.py "$OzOFF_database_path" "$sample_path" "$output_dir"
