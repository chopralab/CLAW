...
#SBATCH --array=0-50  # Adjust this for the number of files to process ######################## <- CHANGE THIS
...
input_dir="Projects/saniya/samples/OFF/"
output_dir="Projects/saniya/match/OFF/"
...
if [ $SLURM_ARRAY_TASK_ID -lt $num_files ]; then
    ...
    # Log the selected sample file for this task
    echo "Processing file: $sample_path" >&2

    # Run the Python script with the input variables
    python core/python/match_3_saniya.py "$OzOFF_database_path" "$sample_path" "$output_dir"

    # Log the completion of this task
    echo "Completed processing file: $sample_path" >&2
else
