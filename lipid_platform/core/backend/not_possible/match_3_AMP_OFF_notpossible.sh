...
#SBATCH --output=core/backend/logs/match/not_possible/match_3_AMP_notpossible_%A_%a_output.txt
#SBATCH --error=core/backend/logs/match/not_possible/match_3_AMP_notpossible_%A_%a_err.txt
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=2
#SBATCH --mem-per-cpu=16G
#SBATCH --time=24:00:00
#SBATCH --array=0-46  # Array indices for 100 jobs, with a check to only process 41 files
...
output_dir="Projects/AMP/match/OFF/notpossible/"
...
if [ $SLURM_ARRAY_TASK_ID -lt ${#files[@]} ]; then
    ...
    python core/python/not_possible/match_3_AMP_OFF_notpossible.py "$OzON_database_path" "$sample_path" "$output_dir"
