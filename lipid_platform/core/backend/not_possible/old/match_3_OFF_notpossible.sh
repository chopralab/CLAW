...
#SBATCH --output=core/backend/logs/match/match_3_NotPossible_%A_%a_output.txt
#SBATCH --error=core/backend/logs/match/match_3_NotPossible_%A_%a_err.txt
#SBATCH --ntasks=1
...
if [ $SLURM_ARRAY_TASK_ID -lt ${#files[@]} ]; then
    ...
    python core/python/not_possible/match_3_OFF_notpossible.py "$OzOFF_database_path" "$sample_path" "$output_dir"
