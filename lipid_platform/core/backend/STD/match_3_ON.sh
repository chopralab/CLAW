...
#SBATCH --output=core/backend/logs/match/match_3_STDon_%A_%a_output.txt
#SBATCH --error=core/backend/logs/match/match_3_STDon_%A_%a_err.txt
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=2
#SBATCH --mem-per-cpu=16G
#SBATCH --time=24:00:00
#SBATCH --array=0  # Array indices for 100 jobs, with a check to only process 41 files
...
# OzOFF_database_path="Projects/STD/off_possible/"
OzOFF_database_path="Projects/STD/analysis/OFF/off_possible/"
...
if [ $SLURM_ARRAY_TASK_ID -lt ${#files[@]} ]; then
    ...
    python core/python/STD/match_3_ON.py "$OzOFF_database_path" "$OzON_database_path" "$sample_path" "$output_dir"
