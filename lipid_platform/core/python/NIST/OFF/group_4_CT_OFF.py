...
class LipidGrouper:
    ...
    def save_grouped_results(self, df, output_dir, sample_value):
        """
        Save the grouped DataFrame to a Parquet file.
        """
        output_file_path = os.path.join(output_dir, f"df_group_4_{sample_value}_OFF.parquet")
        print(f"Saving DataFrame with {len(df)} rows to file: {output_file_path}", file=sys.stderr)
        df.to_parquet(output_file_path, index=False)
        print(f"File successfully saved to: {output_file_path}", file=sys.stderr)
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Group lipid data and save the results.")
    parser.add_argument(
        "--input_file", 
        type=str, 
        required=True, 
    ...
    parser.add_argument(
        ...
        type=str, 
        default="Sample", 
        help="Flag to indicate STD Only processing. Use 'STD' or other values."
    )
...
if __name__ == "__main__":
    ...
    grouper.save_grouped_results(df_grouped, args.output_dir, sample_value)
