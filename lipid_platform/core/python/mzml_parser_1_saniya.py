...
class MzMLParser:
    ...
    def mzml_parser(self, file_path, plot_chromatogram=False):
        ...
        for spectrum in run:
            for element in spectrum.ID.split(' '):
                if 'Q1' in element:
                    ...
                    print(f"Found Q1: {q1_mz}")

                if 'Q3' in element:
                    q3 = element.split('=')
                    q3_mz = np.round(float(q3[1]), 1)
                    print(f"Found Q3: {q3_mz}")
        ...
        if not rows:
            print(f"No rows to append for file: {file_path}")
        else:
            print(f"Appending {len(rows)} rows to transition_summed_df")

        if not ozesi_rows:
            print(f"No OzESI rows to append for file: {file_path}")
        else:
            print(f"Appending {len(ozesi_rows)} rows to OzESI_df")
    ...
    def mzml_parser_batch(self, folder_name, plot_chromatogram=False):
        ...
        print(f"Processing folder: {folder_name} with {len(data_folder)} files")
    ...
    def get_transition_summed_df(self):
        print(f"Returning transition_summed_df with {len(self.transition_summed_df)} rows")
        return self.transition_summed_df

    def get_OzESI_df(self):
        print(f"Returning OzESI_df with {len(self.OzESI_df)} rows")
    ...
    def save_and_measure_size(self, df, file_prefix):
        ...
        try:
            ...
            print(f"Failed to save file at: {file_path}. Error: {e}")
...
if __name__ == "__main__":
    ...
    transition_summed_sizes = parser.save_and_measure_size(transition_summed_df, "Projects/saniya/mzml_parsed/OFF/df_transition_summed_1_OFF")
    OzESI_sizes = parser.save_and_measure_size(OzESI_df, "Projects/saniya/mzml_parsed/OFF/df_mzml_parser_1_OFF")
