...
class MatchLipids:
    ...
    @staticmethod
    def run_match_lipids(OzOFF_database, df_sample_2, tolerance=0.3, retention_time_window=0.5, output=None, match_on_median=False):
        """
        Function to match lipids between df_sample_2 and OzOFF database and filter df_sample_2 data based on retention time.
        ...
        """
        # Initialize the Match_Lipids class
        matcher = MatchLipids(OzOFF_database, tolerance=tolerance)
        ...
        for index, row in tqdm(OzOFF_database.iterrows(), total=len(OzOFF_database), desc="Processing OzOFF Database"):
            ...
            temp_data = filtered_df_sample_2.copy()

            # Match lipids for the current filtered data
            matched_temp_data = matcher.match_lipids_parser(temp_data)
            
            # Append the results to the master DataFrame
            results_df = pd.concat([results_df, matched_temp_data], ignore_index=True)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python match_full_OFF_3.py <OzOFF_database.parquet> <df_sample_2.parquet>")
        sys.exit(1)

    # Parse command-line arguments
    OzOFF_database_path = sys.argv[1]
    df_sample_2_path = sys.argv[2]

    # User-defined variables
    tolerance = 0.3  # Set your tolerance value here
    retention_time_window = 0.5  # Set your retention time window here
    matched_data = "df_match_full_3.parquet"  # Save MatchedData output Parquet file
    ...
    results = MatchLipids.run_match_lipids(
        OzOFF_database, 
        df_sample_2, 
        tolerance=tolerance, 
    ...
    # Print results
    print("Filtered OzOFF Database Matches:")
    print(results.head())
