...
#RIDGE PLOT
  ...
  ggsave(paste("plots/test4/Ridge_Plot_All_Lipids_", title_for_plot, ".pdf", sep=''), plot = ridge_plot)
}
...
ggsave(filename = paste("plots/heatmap/Heatmap_", title_for_plot, ".pdf", sep = ''),
       plot = heatmap_plot)
...
library(dplyr)
library(ggplot2)
library(ggplot2movies)  # Just an example dataset for the loop

# Assuming you have 'pheatmap' package installed and loaded
...
for (current_file in files_in_pre_edgeR) {
  ...
  pheatmap(cor_matrix)  # Plot the heatmap to the PDF file
