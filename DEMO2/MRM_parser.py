...
def make_pie_chart(merged_df,save_path,json_list_singles,labels_list,blank_name,extra_name,show_percentages=False):
...
def average_pie_chart(merged_df,save_path,json_list_singles,labels_list,blank_name,extra_name,show_percentages=False):
...
def read_mrm_list(filename,remove_std = True,custom_data=False):
    if custom_data==False:
        ...
        mrm_list_offical['Transition'] = mrm_list_offical['Parent_Ion'].astype(str) + ' -> ' + mrm_list_offical['Product_Ion'].astype(str)
        try:
            mrm_list_offical['IDs'] = mrm_list_offical['IDs']
            mrm_list_offical['Names'] = mrm_list_offical['Name']
        except:
            pass
        # Create transition column by combining Parent Ion and Product Ion with arrow between numbers
...
def prep_edge_R(merged_df,json_list_pairs,Pre_edge_r_path,blank_name,labels_list,extra_name):
    for i in range(len(json_list_pairs)):
        ...
        num_value_columns_df1 = reformatted_df1.shape[1] - 2  
        num_value_columns_df2 = reformatted_df2.shape[1] - 2 
...
def process_for_biopan(file_path):
    # Load the data
    file_path_orig=file_path
    file_path = file_path+"Pre_EdgeR/"

    for file_name in os.listdir(file_path):
        if file_name.endswith(".csv"):
            full_file_path = os.path.join(file_path, file_name)
            # process_for_biopan(full_file_path)
    #
            ...
            full_path = os.path.join(file_path_orig, "biopan", output_name)
            # Ensure the directory exists
            directory = os.path.dirname(full_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
            df.to_csv(file_path_orig+"biopan/"+output_name, index=False)
